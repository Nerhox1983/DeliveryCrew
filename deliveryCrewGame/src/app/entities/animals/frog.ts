...
/**
 * Representa una rana en el juego.
...
export class Frog extends Animal
{
    ...
    private readonly PREPARE_DURATION: number = 0.2; // Tiempo para rotar antes de saltar
    private jumpTimer: number = 0;
    private readonly jumpDuration: number = 0.4; // Duración del salto en segundos
    private readonly jumpHeight: number = 48;    // Altura del salto en píxeles
    private jumpCooldown: number = 0;
    private readonly JUMP_COOLDOWN_TIME = 1.5; // Segundos de espera entre saltos

    // --- Hambre y Caza ---
    // hunger hereda de Animal
    override chasingTarget: Worm | null = null;
    private aiTimer: number = 0; // Timer propio para la rana
    private readonly AI_INTERVAL: number = 20;

    // --- Deambular ---
    private wanderTimer: number = 0;
    private readonly WANDER_COOLDOWN: number = 3; // Segundos entre saltos aleatorios
    ...
    private readonly JUMP_DISTANCE = 64; // Distancia que recorre en un salto (1 tile)

    constructor(private image: HTMLImageElement, r: number, c: number, config?: SpriteConfig)
    {
        super(image, r, c, config);

        // Ajustes específicos de Rana sobreescritos tras el super()
        this.spriteWidth = 32;
    ...
    /**
     * Actualiza el estado de la rana.
    ...
    private updateHungerLogic(dt: number): void
    {
        ...
        { // Las ranas tienen metabolismo rápido, -1% cada 30s
    ...
    {
        // El spritesheet de la rana tiene un orden diferente: 0:N, 1:W, 2:S, 3:E
